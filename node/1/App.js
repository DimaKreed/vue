...
const createTXT_Files = (pathToTXT, data) => {
    if (!fs.existsSync(pathToTXT)) {
        fs.writeFile(pathToTXT, data, (err) => { if (err) console.log(err); });
    }
};
...
const moveGenderToAnotherDir = (oldPath, newPath, fileName, gender) => {
    const pathToFile = path.join(oldPath, fileName);
    const newPathToFile = path.join(newPath, fileName);
...
for (let i = 0; i < people.length; i++) {
    const pathToTXT_inDir18 = path.join(pathToDir18, `${people[i].name}.txt`);
    // eslint-disable-next-line max-len
    const pathToTXT_inDir20 = path.join(pathToDir20, `${people[i].name}_1.txt`);
    // i have written '_1.txt' cause when i'll move files from dir 1800 to dir 2000 it'll be collision of names
